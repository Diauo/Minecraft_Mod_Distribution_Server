...
from app.services import admin_service, file_service
...
@admin_bp.route('/query_monitor', methods=['POST'])
def get_file_list():
    """
    查询监控列表
    """
    request_body = request.get_json()
    status_code, msg, data = admin_service.query_monitor_files(request_body)
    if status_code != 200:
        return msg, 500
    else:
...
@admin_bp.route('/gen_version', methods=['GET'])
def gen_version():
    """
    生成版本列表
    """
...
@admin_bp.route('/reload_config', methods=['GET'])
def reload_config():
    """
    重载配置文件
    """
...
@admin_bp.route('/get_directory', methods=['POST'])
...
@admin_bp.route('/modify_access_list', methods=['POST'])
...
@admin_bp.route('/get_access_list', methods=['GET'])
def get_access_list():
    """
    获取当前访问控制列表
    """
